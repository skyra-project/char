...
export function isSurrogate(code: number) {
	return isInRange(code, CharUnicodeInfo.HIGH_SURROGATE_START, CharUnicodeInfo.LOW_SURROGATE_END);
}
...
export function isHighSurrogate(code: number) {
	return isInRange(code, CharUnicodeInfo.HIGH_SURROGATE_START, CharUnicodeInfo.HIGH_SURROGATE_END);
}

/** Check if a char is a low surrogate. */
export function isLowSurrogate(code: number) {
	return isInRange(code, CharUnicodeInfo.LOW_SURROGATE_START, CharUnicodeInfo.LOW_SURROGATE_END);
}
...
export function isSurrogatePair(highSurrogate: number, lowSurrogate: number) {
	...
	const highSurrogateOffset = highSurrogate - CharUnicodeInfo.HIGH_SURROGATE_START;
	const lowSurrogateOffset = lowSurrogate - CharUnicodeInfo.LOW_SURROGATE_START;
